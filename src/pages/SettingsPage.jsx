    ...
    <div style={{ marginBottom: 18 }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <input
          ...
          style={{
            width: "100%",
            padding: prefix ? "14px 48px 14px 30px" : "14px 48px 14px 14px",
            borderRadius: 12,
...
export default function SettingsPage({ onNavigate, onLogout }) {
  ...
  const save = async () => {
    ...
    if (isNaN(newRate) || newRate <= 0) {
      showToast("Per unit rate must be > 0", "error");
      return;
    }
    if (isNaN(newWaste) || newWaste < 0) {
      showToast("Waste fee cannot be negative", "error");
      return;
    }
    ...
    >
      ...
      <div style={{ padding: "14px 16px 0" }}>
        ...
        >
          ...
          >
            Rate changes only affect <strong>future billing cycles</strong>. All
            previously generated bills are permanently locked with their
            original rates.
          </p>
        </div>
        ...
        <SectionCard
          icon="⚡"
          title="Electricity & Water Rate"
          subtitle="Applied per combined unit (electricity + water share)"
        >
          {loading ? (
            ...
            <>
              ...
              >
                💡 Example: If a renter uses 30 elec. units + 15 water share =
                45 units × ৳{rate}/unit = ৳
                {(45 * (parseFloat(rate) || 0)).toFixed(2)}
              </div>
            </>
          )}
        </SectionCard>
        ...
        <SectionCard
          ...
          subtitle="Flat monthly fee charged to every active unit"
        >
          {loading ? (
            <div style={{ color: C.textMuted, fontSize: 13 }}>Loading…</div>
          ) : (
        ...
        {history.length > 0 && (
          <SectionCard
            ...
            subtitle="Rate changes made this session"
          >
            {history.map((h, i) => (
              <div
                key={i}
        ...
        <SectionCard
          ...
          subtitle="Units are managed from settings now"
        >
          <button
        ...
        <SectionCard
          ...
          subtitle="Sign out from this session"
        >
          <button
            onClick={() => onLogout?.()}
            style={{
